...
static const char *const TAG = "panasonic_ckp.climate";
/*
const std::map<Protocol, std::function<PanasonicCkpClimate *()>> PROTOCOL_CONSTRUCTOR_MAP = {
...
void PanasonicCkpClimate::setup() {
  ...
  this->heatpump_ir_ = new PanasonicCKPHeatpumpIR();
  this->cancel_time_ = 0;
...
void PanasonicCkpClimate::transmit_state() {
  ...
  uint8_t swing_v_cmd;
  ESP_LOGI(TAG, "transmit_state() start");
  ...
  heatpump_ir_->send(esp_sender, power_mode_cmd, operating_mode_cmd, fan_speed_cmd, temperature_cmd, swing_v_cmd, swing_h_cmd);

  this->cancel_time_ = millis() + (60 * 2 * 1000);

  ESP_LOGI(TAG, "transmit_state() power: %d", power_mode_cmd);
  ESP_LOGI(TAG, "transmit_state() acmode: %d", operating_mode_cmd);
  ESP_LOGI(TAG, "transmit_state() fan: %d", fan_speed_cmd);
  ESP_LOGI(TAG, "transmit_state() temp: %d", temperature_cmd);
  ESP_LOGI(TAG, "transmit_state() vswing: %d", swing_v_cmd);
  ESP_LOGI(TAG, "transmit_state() hswing: %d", swing_h_cmd);
  ESP_LOGI(TAG, "transmit_state() done");
}

void PanasonicCkpClimate::cancel_timer()
{
  IRSenderESPHome esp_sender(this->transmitter_);
  ((PanasonicCKPHeatpumpIR *)heatpump_ir_)->sendPanasonicCKPCancelTimer(esp_sender);
}

void PanasonicCkpClimate::loop() {
  if (this->cancel_time_ > 0 && this->cancel_time_ < millis()) {
    cancel_timer();
    this->cancel_time_ = 0;
  }
}
